...
(function () {
  ...
  const catCorr = [
    ...
    ["홈리빙/가전", -0.29, -0.61, 0.12],
    ["메이크업", -0.30, -0.51, -0.00],
    ["구강용품", -0.32, -0.52, -0.13],
    ["바디케어", -0.32, -0.48, -0.15],
    ["푸드", -0.32, -0.44, 0.07],
    ["헬스/건강용품", -0.32, -0.49, -0.01],
    ["향수/디퓨저", -0.33, -0.59, -0.05],
    ["스킨케어", -0.35, -0.54, -0.14],
    ["헤어케어", -0.35, -0.50, -0.21],
    ["패션", -0.37, -0.67, -0.16],
  ...
  const ratingBins = [
    [0.0, 62], [4.1, 2], [4.2, 1], [4.3, 6], [4.4, 11], [4.5, 40], [4.6, 118], [4.7, 313], [4.8, 784], [4.9, 436], [5.0, 123],
  ];
  ...
  const promo = [
    ["쿠폰 배지", 13.8, 8.8], ["증정 배지", 9.3, 5.8],
    ["할인율 (1%p당)", 1.9, 13.4], ["세일 배지", -4.4, 1.6],
  ];

  /* 그림 8. 이벤트 스터디 — 평소 순위 대비 ln(순위) 편차 (음수 = 상위) */
  const eventDays = [-3, -2, -1, 0, 1, 2, 3];
  const eventSeries = {
    쿠폰: [0.038, 0.048, 0.055, -0.130, -0.033, -0.057, -0.023],
    세일: [0.109, 0.138, 0.166, 0.001, -0.048, -0.077, -0.038],
  };
  ...
  const cloudWords = [
    ["피부", 14016], ["느낌", 12887], ["향", 7200], ["꾸준히", 6166],
    ["촉촉", 5142], ["자극", 5120], ["여름", 4975], ["효과", 4798],
    ["부담", 4403], ["얼굴", 4084], ["재구매", 3988], ["제형", 3939],
    ["데일리", 3841], ["가격", 3716], ["아침", 3400], ["부드럽게", 3229],
    ["메이크업", 3196], ["끈적임", 3008], ["매일", 2875], ["화장", 2819],
    ["지속력", 2632], ["만족", 2583], ["크림", 2565], ["성분", 2545],
    ["거품", 2533], ["시간", 2529], ["선크림", 2498], ["자연스럽게", 2442],
    ["트러블", 2387], ["가성비", 2302], ["가볍게", 2231], ["깔끔하게", 2203],
    ["구성", 2180], ["타입", 2176], ["발림성", 2151], ["도움", 2090],
    ["관리", 2001], ["흡수", 1918], ["세안", 1901], ["나면", 1871],
    ["기획", 1850], ["냄새", 1835], ["부드럽고", 1815], ["진정", 1809],
    ["수분", 1803], ["용량", 1797], ["컬러", 1787], ["편하고", 1753],
  ];

  /* 그림 12. 소구 속성별 언급률 (%) — 전체 및 카테고리별 */
  const attrNames = ["보습·수분", "발림·제형", "자극·순함", "가격·가성비", "향", "끈적임·마무리", "재구매 의사", "트러블·진정", "커버·발색", "지속력"];
  const attrOverall = [20.2, 18.5, 16.5, 15.0, 13.7, 12.8, 12.5, 9.3, 9.2, 9.1];
  const attrProfile = [
    ["메이크업", 7491, [19.1, 19.1, 3.1, 9.7, 3.0, 15.9, 9.7, 6.2, 31.6, 25.5]],
    ["스킨케어", 7083, [46.7, 36.5, 30.2, 14.5, 5.6, 27.8, 15.6, 24.6, 6.0, 6.4]],
    ["마스크팩", 5147, [42.6, 15.5, 26.0, 15.3, 3.7, 11.7, 17.2, 23.2, 18.7, 6.0]],
    ["클렌징", 4848, [26.2, 20.5, 47.4, 13.1, 8.0, 7.9, 14.4, 13.0, 1.5, 2.5]],
    ["선케어", 4737, [32.4, 35.3, 17.8, 12.9, 4.6, 35.9, 11.0, 9.9, 26.0, 6.5]],
    ["헤어케어", 4433, [8.9, 20.4, 9.9, 13.9, 34.5, 12.3, 12.9, 2.8, 3.0, 10.2]],
    ["푸드", 3752, [1.3, 3.2, 3.1, 17.4, 6.2, 0.7, 14.0, 0.5, 0.2, 1.8]],
    ["바디케어", 3504, [27.9, 19.4, 19.2, 11.9, 44.1, 18.5, 11.1, 6.8, 4.1, 8.6]],
    ["건강식품", 3344, [2.1, 4.2, 2.2, 19.9, 3.9, 0.4, 14.7, 0.7, 0.1, 3.0]],
    ["위생용품", 3268, [9.4, 22.5, 27.1, 22.0, 17.1, 7.4, 15.9, 1.5, 4.6, 5.6]],
    ["뷰티소품", 2946, [4.5, 16.1, 13.3, 20.5, 1.2, 2.9, 12.3, 2.2, 8.8, 5.2]],
    ["맨즈에딧", 2807, [20.7, 17.2, 12.5, 13.1, 18.9, 19.4, 10.7, 7.7, 8.4, 10.5]],
    ["네일", 2760, [14.1, 20.3, 4.4, 14.5, 16.3, 8.5, 8.3, 0.5, 6.0, 14.9]],
    ["헬스/건강용품", 2759, [5.2, 4.8, 10.0, 16.3, 7.2, 2.6, 11.0, 19.8, 8.3, 4.2]],
    ["구강용품", 2732, [1.8, 5.1, 12.8, 17.5, 16.5, 1.3, 13.9, 0.1, 0.5, 7.1]],
    ["향수/디퓨저", 2644, [2.2, 4.2, 3.8, 16.8, 79.9, 4.2, 9.3, 0.5, 0.3, 32.0]],
    ["더모 코스메틱", 2590, [48.3, 33.4, 33.9, 13.1, 9.4, 21.4, 12.4, 26.8, 5.4, 7.2]],
    ["패션", 1870, [1.1, 5.2, 5.3, 16.0, 0.4, 1.2, 8.0, 0.4, 6.0, 1.6]],
    ["홈리빙/가전", 1389, [4.1, 3.2, 5.7, 15.8, 27.2, 1.2, 9.7, 0.5, 0.4, 6.0]],
    ["취미/팬시", 949, [0.4, 4.0, 1.2, 18.1, 0.6, 0.6, 6.1, 0.1, 0.3, 2.4]],
  ];
  ...
  const discTypes = [
    ["가속형", "s1", [0, 0.13, 0.07, 0.25, 0.43, 0.53], [0, 0.44, 0.51, 0.95, 1.21, 1.18], 26, 1973],
    ["데스밸리형", "s3", [0, -0.25, -0.21, -0.04, 0.14, 0.24], [0, -0.60, -0.49, -0.26, 0.00, -0.24], 12, 982],
    ["포화형", "s2", [0, 0.12, 0.09, -0.01, -0.09, -0.17], [0, 0.28, 0.55, 0.03, -0.27, -0.17], 8, 685],
    ["무반응·역행형", "neutralInk", [0, -0.23, -0.31, -0.29, -0.24, -0.37], [0, -0.17, -0.09, -0.21, -0.12, -0.47], 14, 1071],
  ];

  /* 그림 10. 프로모션 온셋 전후 판매지수 — 실제와 반사실 (사전 평균 = 100) */
  const cfDays = ["D-7", "D-6", "D-5", "D-4", "D-3", "D-2", "D-1", "D+0", "D+1", "D+2", "D+3", "D+4"];
  const cfPre = [83.0, 107.3, 108.3, 104.6, 110.2, 112.2, 79.7];
  const cfPost = {
    "실제(Actual)": [212.9, 72.3, 222.8, 146.6, 137.7],
    "EARTH(MARS)": [190.3, 79.9, 198.7, 140.7, 133.9],
    "신경망(MLP)": [205.5, 78.2, 198.1, 143.7, 135.5],
    "시장대조(DiD)": [201.8, 73.8, 212.7, 144.8, 136.3],
    "ARIMA(1,0,0)": [110.0, 96.1, 107.0, 99.3, 100.0],
    "사전평균(무보정)": [100.0, 100.7, 102.3, 100.9, 98.0],
  };

  const cfStyles = [
    ["실제(Actual)", "ink", 3.2, null],
    ["EARTH(MARS)", "s1", 2.2, "6 4"],
    ["신경망(MLP)", "s2", 2, "6 4"],
  ...
  const revCat = [
    ["취미/팬시", 89.6, 5.8, 4.32, 28],
    ["네일", 91.7, 5.7, 2.69, 85],
    ["패션", 92.3, 4.8, 2.99, 112],
    ["헬스/건강용품", 93.6, 4.5, 1.85, 213],
    ["메이크업", 93.9, 4.2, 1.92, 232],
    ["선케어", 94.3, 3.7, 2.10, 224],
    ["뷰티소품", 94.4, 3.9, 1.53, 254],
    ["구강용품", 94.4, 4.0, 1.44, 272],
    ["푸드", 94.7, 3.6, 1.61, 257],
    ["헤어케어", 95.0, 3.5, 1.33, 251],
    ["향수/디퓨저", 95.1, 3.2, 1.61, 251],
    ["바디케어", 95.2, 3.4, 1.27, 229],
    ["마스크팩", 95.4, 3.3, 1.29, 249],
    ["홈리빙/가전", 95.4, 2.9, 1.61, 56],
    ["더모 코스메틱", 95.7, 3.1, 1.13, 244],
    ["클렌징", 95.7, 3.1, 1.07, 213],
    ["스킨케어", 95.8, 3.0, 1.14, 183],
    ["맨즈에딧", 95.9, 2.8, 1.00, 234],
    ["건강식품", 96.0, 3.0, 0.81, 304],
    ["위생용품", 96.2, 2.7, 0.87, 215],
  ];

  /* 그림 14. 브랜드별 긍정 비중 산포 — [브랜드, 긍정 비중%, 브랜드 내 상품 간 표준편차%p, 상품수] */
  const revBrand = [
    ["웰릭스",85.80,6.14,5], ["쏘두위",86.20,9.96,5], ["라이브오랄스",88.60,3.05,5], ["베리시",88.75,7.70,12], ["바나실",89.60,5.03,5], ["마르시끄",89.73,2.49,11],
    ["알로",90.50,2.59,6], ["위드샨",90.69,3.28,16], ["니들리",90.80,5.63,5], ["체이싱래빗",90.86,2.61,7], ["오설록",91.00,3.54,5], ["달리프",91.20,3.27,5],
    ["민티드",91.20,2.59,5], ["센시안",91.55,5.26,11], ["이지덤뷰티",91.57,3.78,7], ["마른파이브",91.57,3.25,14], ["리무브",91.78,2.82,9], ["해서린",92.00,4.00,5],
    ["에이딕트",92.12,2.95,8], ["그로우어스",92.20,2.95,5], ["셀라딕스",92.40,1.67,5], ["디오디너리",92.40,2.07,5], ["웨이크메이크",92.50,2.19,28], ["케어플러스",92.61,4.92,23],
    ["필리밀리",92.65,4.97,82], ["투크",92.71,3.20,7], ["비플레인",92.74,7.63,19], ["나르카",92.78,1.92,9], ["유시몰",92.79,3.86,29], ["파넬",92.83,3.25,6],
    ["크런틴",92.83,3.82,6], ["반디",92.90,3.35,10], ["메디큐브",92.91,2.75,23], ["어바웃톤",93.00,2.12,5], ["센녹",93.00,2.35,5], ["힌스",93.00,2.00,9],
    ["그린몬스터",93.17,2.71,6], ["스노우투플러스",93.20,3.56,5], ["다슈",93.29,4.34,31], ["리브러쉬",93.30,2.26,10], ["푸드올로지",93.33,2.82,24], ["비너스",93.33,2.58,6],
    ["쏘내추럴",93.38,3.11,8], ["발란스핏",93.40,2.51,5], ["차앤박",93.40,4.39,5], ["데싱디바",93.43,2.76,7], ["뷰센",93.54,3.26,13], ["네이밍",93.57,2.07,7],
    ["아리얼",93.60,4.67,10], ["테일러",93.60,2.88,5], ["클로란",93.60,1.67,5], ["식물나라",93.61,2.79,33], ["프로티원",93.62,3.62,8], ["청미정",93.62,1.69,8],
    ["나르시소 로드리게즈",93.67,2.66,6], ["컬러그램",93.71,1.38,7], ["나트라케어",93.80,2.49,5], ["grn+",93.80,1.48,5], ["엘리자베스아덴",93.83,2.14,6], ["플르부아",93.86,1.35,7],
    ["퓌",93.89,1.54,9], ["케라스타즈",93.89,1.90,9], ["페리오",93.92,2.91,12], ["아뜰리에페이",93.92,2.93,13], ["프롬리에",94.00,2.76,6], ["박준뷰티랩",94.00,2.55,9],
    ["코자아",94.00,2.45,6], ["칼로(Kalo)",94.00,1.22,5], ["코스노리",94.00,3.00,7], ["플라이밀",94.12,3.04,8], ["홀리카홀리카",94.17,2.71,6], ["스팀베이스",94.20,2.77,5],
    ["아떼",94.25,1.98,8], ["아누아",94.27,2.36,37], ["로레알",94.30,2.26,10], ["일소",94.31,1.84,13], ["라운드어라운드",94.31,2.25,13], ["에뛰드",94.36,2.27,14],
    ["릴리바이레드",94.40,1.52,5], ["낫띵베럴",94.40,1.67,5], ["투에이엔",94.40,3.21,5], ["라로슈포제",94.47,2.32,38], ["피카소",94.50,2.61,16], ["넥스케어",94.50,3.70,8],
    ["조선미녀",94.56,2.13,9], ["달바",94.56,3.05,16], ["팁토우",94.60,1.95,5], ["카이",94.60,2.07,5], ["리터니티",94.60,2.41,5], ["메디필",94.60,1.96,10],
    ["테라브레스",94.62,4.99,13], ["브링그린",94.64,2.02,28], ["투쿨포스쿨",94.67,1.75,6], ["이지듀",94.70,1.77,10], ["더툴랩",94.70,1.57,10], ["유세린",94.71,1.76,17],
    ["낫포유",94.71,2.06,7], ["올리브영",94.72,3.12,43], ["바른생각",94.75,1.98,16], ["정샘물",94.78,1.48,9], ["어뮤즈",94.80,2.17,5], ["셀렉스",94.80,2.68,5],
    ["동국제약",94.83,3.19,6], ["멈칫",94.83,2.23,6], ["플랙커스",94.83,6.31,6], ["티젠",94.83,1.99,12], ["탄탄",94.83,1.47,6], ["이즈앤트리",94.83,1.33,6],
    ["버버리",94.83,2.14,6], ["토르홉",94.88,1.46,8], ["에스쁘아",94.89,2.15,9], ["한율",94.90,0.74,10], ["페리페라",94.90,2.02,10], ["롬앤",94.90,2.13,10],
    ["라보에이치",94.91,0.94,11], ["바이오던스",94.94,2.22,17], ["보다나",95.00,3.55,8], ["존슨즈",95.00,2.68,6], ["크리스탈",95.00,0.00,5], ["오니스트",95.00,1.10,6],
    ["듀이트리",95.00,2.29,14], ["클리오",95.07,1.38,14], ["그라펜",95.09,3.36,11], ["스트라이덱스",95.14,1.86,7], ["코링코",95.14,0.38,7], ["루치펠로",95.15,2.18,20],
    ["셀퓨전씨",95.17,1.40,12], ["라네즈",95.18,2.75,11], ["리필드",95.20,1.79,5], ["딜라이트 프로젝트",95.20,2.30,35], ["니베아",95.22,1.86,9], ["올더베러",95.24,1.41,21],
    ["미쟝센",95.25,2.24,16], ["바이탈뷰티",95.25,1.75,8], ["헤트라스",95.25,1.58,8], ["화이트",95.25,2.90,12], ["아비브",95.26,2.24,23], ["해피바스",95.29,2.29,7],
    ["이너시아",95.30,2.00,10], ["구달",95.32,2.12,22], ["클린",95.33,1.58,9], ["에스더포뮬러",95.40,0.55,5], ["닥터포헤어",95.40,1.26,10], ["이니스프리",95.42,3.32,12],
    ["락토핏",95.42,3.20,12], ["아비노",95.43,1.51,7], ["덴티스테",95.45,2.74,20], ["코스알엑스",95.45,2.77,11], ["바닐라코",95.47,2.82,19], ["닥터자르트",95.50,2.15,12],
    ["에스네이처",95.55,1.30,22], ["프리메라",95.56,1.13,9], ["넘버즈인",95.58,0.97,24], ["오랄비",95.60,1.52,5], ["바이오가",95.62,1.85,8], ["가그린",95.62,2.07,8],
    ["아벤느",95.64,1.28,14], ["아로마티카",95.67,2.00,9], ["마녀공장",95.67,1.15,12], ["바이오더마",95.68,1.88,38], ["메이크프렘",95.73,2.02,15], ["스킨푸드",95.75,1.04,8],
    ["바이오코어유산균",95.75,1.28,8], ["라엘",95.77,2.01,13], ["바이오힐보",95.77,1.97,22], ["아렌시아",95.80,2.17,5], ["닥터디퍼런트",95.80,1.79,5], ["지미추",95.83,1.47,6],
    ["제로이드",95.85,0.95,27], ["아크네스",95.86,0.69,7], ["멜라메이트",95.86,0.38,7], ["위시어",95.86,0.38,7], ["큐라덴",95.86,1.86,7], ["에스트라",95.95,1.38,40],
    ["메디힐",96.00,1.70,37], ["랑방",96.00,1.26,11], ["헤라",96.00,1.66,9], ["키스미",96.00,2.28,6], ["어노브",96.00,2.03,16], ["스튜디오17",96.00,1.83,13],
    ["나른",96.08,1.75,13], ["쉬크",96.12,1.13,8], ["츠바키",96.12,0.64,8], ["리쥬란",96.13,1.55,15], ["아리아나그란데",96.17,0.75,6], ["센소다인",96.17,0.75,6],
    ["레이비",96.17,0.75,6], ["갸스비",96.18,2.04,11], ["닥터지",96.19,1.66,27], ["유리아쥬",96.20,3.19,10], ["바이컬러",96.20,0.84,5], ["토리든",96.22,2.10,36],
    ["세타필",96.25,1.98,8], ["라곰",96.25,1.28,8], ["라운드랩",96.31,1.60,48], ["비비랩",96.33,1.23,15], ["쏘피",96.33,1.35,15], ["셀리맥스",96.33,1.73,9],
    ["비욘드",96.33,1.63,6], ["네이처메이드",96.33,1.03,6], ["올리오",96.33,1.41,9], ["폴프랜즈",96.38,1.19,8], ["존바바토스",96.40,0.89,5], ["힐링버드",96.40,0.89,5],
    ["센텔리안24",96.40,1.14,5], ["닥터텅스",96.40,2.41,5], ["피지오겔",96.42,1.53,24], ["피노",96.43,0.79,7], ["리스테린",96.50,1.40,14], ["디어스킨",96.50,2.27,8],
    ["일리윤",96.55,1.57,29], ["세노비스",96.59,1.77,17], ["아임비타",96.60,1.14,5], ["바세린",96.60,3.21,5], ["프롬랩스",96.60,1.14,5], ["캘빈클라인",96.60,1.07,10],
    ["아이디얼포맨",96.60,2.13,15], ["웰라쥬",96.62,0.92,8], ["오브제",96.68,3.05,22], ["포맨트",96.71,1.49,17], ["비페스타",96.71,1.25,7], ["파티온",96.75,1.22,12],
    ["AHC",96.82,1.08,11], ["휴족시간",96.83,0.75,6], ["스킨1004",97.00,1.22,5], ["아이오페",97.06,1.09,17], ["스너글",97.08,0.90,12], ["아토팜",97.25,0.89,8],
    ["센카",97.29,1.38,7], ["유기농본",97.29,0.49,7], ["비레디",97.29,1.60,7], ["VT",97.36,1.45,14], ["우르오스",97.36,0.67,11], ["순수한면",97.38,1.69,8],
    ["CJ이너비",97.50,1.64,6], ["크리넥스",97.56,1.33,9], ["좋은느낌",97.60,0.76,25], ["멕리듬",97.60,0.89,5], ["뉴오리진",97.60,0.89,5], ["고려은단",97.67,0.82,15],
    ["동아제약",97.67,1.37,6], ["오쏘몰",97.71,0.49,7], ["정관장",97.71,0.76,7], ["시루콧토",97.71,0.95,7], ["질레트",97.74,0.99,19], ["덴프스",97.80,1.14,10],
    ["더마비",97.80,0.45,5], ["리뉴",97.83,0.41,6], ["폰즈",98.00,0.00,5], ["커리쉴",98.00,1.22,5], ["옵티프리",98.57,0.79,7],
  ];
  ...
  function drawCfPath() {
    ...
    const act = cfPost["실제(Actual)"], nn = cfPost["EARTH(MARS)"];
  ...
  function drawAll() {
    ...
    const col = C();
    legend("lgEvent", [["쿠폰 부착 (2,917건)", col.s1, "line"], ["세일 부착 (999건)", col.s3, "line"]]);
