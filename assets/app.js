...
(function () {
  ...
  const saleSeries = [["08-18",19.2,34.6],["08-19",19.0,32.6],["08-20",18.9,33.1],["08-21",18.7,32.2],["08-22",18.2,29.9],["08-24",17.9,29.9],["08-25",18.5,28.0],["08-26",18.4,27.9],["08-27",18.3,30.0],["08-28",17.4,26.8],["08-29",16.7,23.8],["08-30",27.0,90.6],["08-31",26.9,90.5],["09-01",26.7,89.4],["09-02",27.3,90.5],["09-03",26.8,89.8],["09-04",27.2,91.0],["09-05",27.1,90.1],["09-06",16.1,17.1],["09-07",18.0,24.3],["09-08",17.6,23.8],["09-09",18.1,24.6],["09-10",18.1,25.3],["09-11",18.7,32.7],["09-12",18.6,31.4],["09-13",18.5,31.7],["09-15",18.6,33.3]];
  ...
  const catCorr = [
    ["홈리빙/가전", -0.28, -0.61, 0.12],
    ["취미/팬시", -0.29, -0.45, -0.03],
    ["메이크업", -0.31, -0.51, -0.00],
    ["푸드", -0.31, -0.44, 0.07],
    ["구강용품", -0.32, -0.52, -0.13],
    ["바디케어", -0.32, -0.48, -0.15],
    ["향수/디퓨저", -0.32, -0.59, -0.05],
    ["헬스/건강용품", -0.32, -0.49, -0.01],
    ["스킨케어", -0.36, -0.54, -0.14],
    ["헤어케어", -0.36, -0.57, -0.21],
    ["패션", -0.37, -0.67, -0.16],
    ["더모 코스메틱", -0.39, -0.60, -0.05],
    ["맨즈에딧", -0.39, -0.53, -0.20],
    ["건강식품", -0.40, -0.57, -0.25],
    ["마스크팩", -0.41, -0.62, -0.29],
    ["뷰티소품", -0.42, -0.56, -0.12],
  ...
  const ratingBins = [
    [0.0, 856], [4.1, 10], [4.2, 24], [4.3, 54], [4.4, 74], [4.5, 176], [4.6, 408], [4.7, 1147], [4.8, 2640], [4.9, 3113],
  ];

  /* 그림 6. 표준화 영향력 (탄력성 절댓값) */
  const elasticity = [["리뷰 증가 속도 (판매량 대리지표)", 0.42, "s1"], ["실제 판매가격", 0.38, "s2"]];

  /* 표 7. 동일 상품 내 프로모션별 순위 변화율 (+ = 개선) */
  const promo = [
    ["쿠폰 배지", 13.8, 9.2], ["증정 배지", 9.2, 5.7],
    ["할인율 (1%p당)", 1.8, 13.1], ["세일 배지", -3.2, 1.3],
  ];

  /* 그림 8. 이벤트 스터디 — 평소 순위 대비 ln(순위) 편차 (음수 = 상위) */
  const eventDays = [-3, -2, -1, 0, 1, 2, 3];
  const eventSeries = {
    쿠폰: [0.020, 0.038, 0.045, -0.144, -0.036, -0.050, -0.021],
    세일: [0.128, 0.151, 0.170, 0.007, -0.049, -0.058, -0.026],
  };
  ...
  const cloudWords = [
    ["피부", 14360], ["느낌", 13181], ["향", 7293], ["꾸준히", 6332],
    ["촉촉", 5303], ["자극", 5225], ["여름", 5028], ["효과", 4897],
    ["부담", 4470], ["얼굴", 4208], ["재구매", 4062], ["제형", 4040],
    ["데일리", 3920], ["가격", 3803], ["아침", 3469], ["부드럽게", 3305],
    ["메이크업", 3261], ["끈적임", 3055], ["매일", 2932], ["화장", 2893],
    ["지속력", 2687], ["크림", 2655], ["만족", 2626], ["성분", 2597],
    ["시간", 2567], ["거품", 2561], ["선크림", 2544], ["자연스럽게", 2507],
    ["트러블", 2445], ["가성비", 2348], ["가볍게", 2286], ["구성", 2245],
    ["깔끔하게", 2238], ["타입", 2230], ["발림성", 2209], ["도움", 2143],
    ["관리", 2060], ["흡수", 1949], ["세안", 1946], ["기획", 1921],
    ["나면", 1920], ["냄새", 1870], ["수분", 1864], ["컬러", 1862],
    ["진정", 1861], ["부드럽고", 1853], ["용량", 1843], ["편하고", 1809],
  ];

  /* 그림 12. 소구 속성별 언급률 (%) — 전체 및 카테고리별 */
  const attrNames = ["보습·수분", "발림·제형", "자극·순함", "가격·가성비", "향", "끈적임·마무리", "재구매 의사", "트러블·진정", "커버·발색", "지속력"];
  const attrOverall = [20.3, 18.5, 16.5, 15.0, 13.6, 12.8, 12.5, 9.3, 9.3, 9.1];
  const attrProfile = [
    ["메이크업", 7838, [18.9, 19.3, 3.2, 10.0, 3.1, 15.8, 9.8, 6.0, 31.5, 25.1]],
    ["스킨케어", 7313, [46.9, 36.1, 29.8, 14.5, 5.5, 27.6, 15.5, 24.3, 5.9, 6.3]],
    ["마스크팩", 5397, [42.8, 15.7, 26.3, 15.5, 3.6, 11.6, 17.0, 23.2, 19.0, 5.9]],
    ["클렌징", 4933, [25.9, 20.5, 47.4, 13.2, 8.1, 7.9, 14.6, 12.9, 1.5, 2.5]],
    ["선케어", 4886, [32.5, 35.4, 17.6, 12.6, 4.6, 35.9, 11.0, 9.7, 26.2, 6.4]],
    ["헤어케어", 4539, [8.8, 20.2, 9.9, 13.8, 34.8, 12.1, 12.7, 2.9, 3.0, 10.3]],
    ["푸드", 3891, [1.2, 3.3, 3.1, 17.4, 6.2, 0.6, 14.0, 0.5, 0.2, 1.7]],
    ["바디케어", 3527, [27.9, 19.4, 19.1, 12.0, 44.3, 18.4, 11.1, 6.7, 4.1, 8.5]],
    ["건강식품", 3482, [2.0, 4.2, 2.2, 19.7, 4.0, 0.4, 14.9, 0.6, 0.1, 3.0]],
    ["위생용품", 3306, [9.3, 22.5, 27.1, 22.1, 17.0, 7.4, 15.9, 1.5, 4.6, 5.6]],
    ["뷰티소품", 2977, [4.5, 15.9, 13.2, 20.6, 1.2, 2.9, 12.2, 2.2, 8.7, 5.2]],
    ["맨즈에딧", 2807, [21.1, 17.4, 12.8, 13.1, 19.1, 19.6, 10.7, 7.8, 8.4, 10.6]],
    ["헬스/건강용품", 2803, [5.1, 4.7, 9.8, 16.3, 7.4, 2.5, 10.9, 19.7, 8.3, 4.2]],
    ["구강용품", 2794, [1.8, 5.1, 12.8, 17.4, 16.5, 1.3, 13.9, 0.1, 0.5, 7.0]],
    ["더모 코스메틱", 2760, [47.5, 33.7, 33.9, 13.9, 9.1, 21.0, 12.1, 27.1, 5.3, 7.1]],
    ["네일", 2760, [14.1, 20.3, 4.4, 14.5, 16.3, 8.5, 8.3, 0.5, 6.0, 14.9]],
    ["향수/디퓨저", 2680, [2.1, 4.3, 3.8, 17.1, 79.8, 4.1, 9.3, 0.4, 0.3, 32.0]],
    ["패션", 1871, [1.1, 5.2, 5.3, 16.0, 0.5, 1.2, 8.0, 0.4, 6.0, 1.6]],
    ["홈리빙/가전", 1407, [4.1, 3.2, 5.9, 15.6, 27.3, 1.1, 9.7, 0.5, 0.4, 5.9]],
    ["취미/팬시", 950, [0.4, 4.0, 1.2, 18.1, 0.6, 0.6, 6.1, 0.1, 0.3, 2.4]],
  ];
  ...
  const discTypes = [
    ["가속형", "s1", [0, 0.09, 0.05, 0.26, 0.43, 0.46], [0, 0.37, 0.54, 0.93, 1.21, 1.04], 28, 2270],
    ["데스밸리형", "s3", [0, -0.22, -0.18, -0.13, 0.13, 0.37], [0, -0.59, -0.63, -0.36, -0.01, -0.11], 8, 691],
    ["포화형", "s2", [0, 0.19, 0.06, -0.02, -0.08, -0.05], [0, 0.43, 0.43, 0.05, -0.09, 0.21], 10, 860],
    ["무반응·역행형", "neutralInk", [0, -0.28, -0.30, -0.27, -0.20, -0.37], [0, -0.19, -0.07, -0.16, -0.10, -0.49], 14, 1121],
  ];

  /* 그림 10. 프로모션 온셋 전후 판매지수 — 실제와 반사실 (사전 평균 = 100) */
  const cfDays = ["D-7", "D-6", "D-5", "D-4", "D-3", "D-2", "D-1", "D+0", "D+1", "D+2", "D+3", "D+4"];
  const cfPre = [85.2, 110.6, 106.9, 106.9, 107.1, 108.0, 79.9];
  const cfPost = {
    "실제(Actual)": [189.9, 71.4, 193.1, 135.2, 121.7],
    "신경망(MLP)": [182.9, 76.6, 172.2, 127.5, 115.0],
    "시장대조(DiD)": [180.7, 72.5, 184.9, 132.8, 120.6],
    "EARTH(MARS)": [165.2, 74.7, 167.2, 123.8, 115.0],
    "ARIMA(1,0,0)": [109.4, 96.1, 106.3, 99.1, 96.9],
    "사전평균(무보정)": [100.0, 100.6, 101.9, 100.8, 94.0],
  };
  /* 음영(순수 증분)의 기준이 되는 반사실 — 홀드아웃 검증 1위 모형 */
  const cfBase = "신경망(MLP)";

  const cfStyles = [
    ["실제(Actual)", "ink", 3.2, null],
    ["신경망(MLP)", "s1", 2.2, "6 4"],
    ["시장대조(DiD)", "s2", 2, "6 4"],
    ["EARTH(MARS)", "wine", 1.8, "3 4"],
  ...
  const revCat = [
    ["취미/팬시", 89.6, 5.9, 4.36, 28],
    ["네일", 91.7, 5.7, 2.68, 85],
    ["패션", 92.4, 4.8, 2.96, 113],
    ["헬스/건강용품", 93.4, 4.6, 1.86, 222],
    ["메이크업", 93.9, 4.3, 1.90, 246],
    ["선케어", 94.4, 3.6, 2.08, 234],
    ["뷰티소품", 94.5, 3.9, 1.53, 261],
    ["구강용품", 94.5, 4.0, 1.41, 287],
    ["푸드", 94.7, 3.6, 1.63, 273],
    ["향수/디퓨저", 94.9, 3.3, 1.68, 263],
    ["헤어케어", 95.1, 3.5, 1.30, 260],
    ["바디케어", 95.3, 3.3, 1.20, 240],
    ["마스크팩", 95.4, 3.4, 1.28, 263],
    ["홈리빙/가전", 95.5, 2.9, 1.59, 56],
    ["더모 코스메틱", 95.7, 3.1, 1.11, 254],
    ["클렌징", 95.7, 3.1, 1.06, 214],
    ["스킨케어", 95.8, 3.0, 1.12, 196],
    ["맨즈에딧", 95.9, 2.8, 1.03, 236],
    ["건강식품", 96.0, 3.0, 0.81, 311],
    ["위생용품", 96.2, 2.7, 0.88, 220],
  ];

  /* 그림 14. 브랜드별 긍정 비중 산포 — [브랜드, 긍정 비중%, 브랜드 내 상품 간 표준편차%p, 상품수] */
  const revBrand = [
    ["웰릭스",85.60,6.47,5], ["쏘두위",86.20,9.96,5], ["라이브오랄스",88.60,3.05,5], ["베리시",88.67,7.67,12], ["바나실",89.60,5.03,5], ["마르시끄",89.64,2.38,11],
    ["알로",90.67,2.34,6], ["아이소이",90.80,5.45,5], ["위드샨",90.81,3.23,16], ["체이싱래빗",90.86,2.61,7], ["민티드",91.20,2.59,5], ["달리프",91.20,3.27,5],
    ["오설록",91.20,3.35,5], ["이지덤뷰티",91.29,3.73,7], ["해서린",91.50,3.78,6], ["센시안",91.55,5.26,11], ["마른파이브",91.57,3.44,14], ["니들리",91.83,5.64,6],
    ["에이딕트",91.88,2.90,8], ["리무브",91.89,2.76,9], ["디오디너리",92.20,2.05,5], ["그로우어스",92.20,2.95,5], ["셀라딕스",92.40,1.67,5], ["웨이크메이크",92.55,2.06,29],
    ["케어플러스",92.62,4.91,24], ["필리밀리",92.64,4.90,84], ["나르카",92.67,1.87,9], ["유시몰",92.70,3.86,30], ["투크",92.71,3.20,7], ["아뜰리에페이",92.73,5.12,15],
    ["비플레인",92.79,7.66,19], ["센녹",92.80,2.39,5], ["반디",92.80,3.19,10], ["크런틴",92.83,3.82,6], ["메디큐브",92.91,2.75,23], ["파넬",93.00,3.00,7],
    ["힌스",93.00,2.00,9], ["그린몬스터",93.17,2.71,6], ["쏘내추럴",93.25,3.20,8], ["데싱디바",93.29,2.56,7], ["다슈",93.29,4.34,31], ["비너스",93.33,2.58,6],
    ["네이밍",93.38,2.00,8], ["푸드올로지",93.38,2.73,24], ["발란스핏",93.40,2.51,5], ["스노우투플러스",93.40,3.36,5], ["투쿨포스쿨",93.44,2.35,9], ["어바웃톤",93.50,2.26,6],
    ["클로란",93.60,1.67,5], ["아리얼",93.60,4.67,10], ["테일러",93.60,2.88,5], ["청미정",93.62,1.69,8], ["리브러쉬",93.64,2.25,11], ["식물나라",93.64,2.76,33],
    ["나르시소 로드리게즈",93.67,2.66,6], ["뷰센",93.69,3.15,13], ["컬러그램",93.71,1.38,7], ["케라스타즈",93.78,1.92,9], ["더블유드레스룸",93.80,1.64,5], ["나트라케어",93.80,2.49,5],
    ["grn+",93.80,1.48,5], ["프롬리에",93.83,2.71,6], ["엘리자베스아덴",93.83,2.14,6], ["플르부아",93.86,1.35,7], ["프로티원",93.88,3.04,8], ["퓌",93.89,1.45,9],
    ["페리오",93.92,2.91,12], ["박준뷰티랩",94.00,2.55,9], ["칼로(Kalo)",94.00,1.22,5], ["코자아",94.00,2.45,6], ["아누아",94.11,2.40,38], ["플라이밀",94.12,3.04,8],
    ["차앤박",94.12,3.52,8], ["홀리카홀리카",94.17,2.71,6], ["탄탄",94.17,3.06,6], ["순백수",94.20,0.45,5], ["스팀베이스",94.20,2.77,5], ["아떼",94.25,1.98,8],
    ["에뛰드",94.29,2.27,14], ["로레알",94.30,2.26,10], ["일소",94.31,1.84,13], ["넥스케어",94.38,4.03,8], ["라운드어라운드",94.38,2.06,13], ["투에이엔",94.40,3.21,5],
    ["리터니티",94.40,2.70,5], ["릴리바이레드",94.40,1.52,5], ["낫띵베럴",94.40,1.67,5], ["코스노리",94.50,3.12,8], ["어뮤즈",94.50,2.07,6], ["메디필",94.55,1.86,11],
    ["조선미녀",94.56,2.13,9], ["달바",94.56,3.05,16], ["팁토우",94.60,1.95,5], ["카이",94.60,2.07,5], ["브링그린",94.61,2.02,28], ["라로슈포제",94.62,2.32,39],
    ["티젠",94.64,2.59,14], ["에스쁘아",94.70,1.95,10], ["이지듀",94.70,1.77,10], ["피카소",94.71,2.66,17], ["낫포유",94.71,2.06,7], ["테라브레스",94.73,4.64,15],
    ["바른생각",94.75,1.84,16], ["유세린",94.78,1.90,18], ["정샘물",94.78,1.48,9], ["토르홉",94.78,1.20,9], ["올리브영",94.80,3.12,44], ["셀렉스",94.80,2.68,5],
    ["바이오던스",94.82,2.19,17], ["플랙커스",94.83,6.31,6], ["존슨즈",94.83,2.71,6], ["멈칫",94.83,2.23,6], ["동국제약",94.83,3.19,6], ["한율",94.90,0.74,10],
    ["페리페라",94.90,2.02,10], ["더툴랩",94.91,1.51,11], ["라보에이치",94.91,0.94,11], ["크리스탈",95.00,0.00,5], ["듀이트리",95.00,2.29,14], ["오니스트",95.00,1.10,6],
    ["자일리톨스톤",95.00,0.71,5], ["이즈앤트리",95.00,1.41,7], ["버버리",95.00,2.19,6], ["보다나",95.00,3.55,8], ["롬앤",95.10,2.18,10], ["니베아",95.11,1.69,9],
    ["코링코",95.14,0.38,7], ["스트라이덱스",95.14,1.86,7], ["셀퓨전씨",95.17,1.64,12], ["에스더포뮬러",95.17,0.75,6], ["그라펜",95.17,3.21,12], ["딜라이트 프로젝트",95.17,2.28,35],
    ["리필드",95.20,1.79,5], ["아비브",95.21,2.26,24], ["클리오",95.21,1.37,14], ["올더베러",95.23,1.38,22], ["라네즈",95.25,2.63,12], ["화이트",95.25,2.86,12],
    ["해피바스",95.29,2.29,7], ["덴티스테",95.32,2.71,22], ["바닐라코",95.33,2.78,21], ["클린",95.33,1.58,9], ["미쟝센",95.35,2.21,17], ["이너시아",95.36,1.91,11],
    ["구달",95.36,2.15,22], ["루치펠로",95.38,2.36,21], ["닥터포헤어",95.40,1.26,10], ["이니스프리",95.42,3.32,12], ["아비노",95.43,1.51,7], ["헤트라스",95.44,1.81,9],
    ["코스알엑스",95.45,2.77,11], ["닥터자르트",95.50,2.15,12], ["콜게이트",95.50,2.74,6], ["오랄비",95.50,1.38,6], ["바이탈뷰티",95.56,1.88,9], ["락토핏",95.58,2.97,12],
    ["에스네이처",95.59,1.26,22], ["아벤느",95.60,1.24,15], ["넘버즈인",95.62,1.06,24], ["바이오가",95.62,1.85,8], ["바이오더마",95.66,1.88,41], ["프리메라",95.67,1.00,9],
    ["아로마티카",95.67,2.00,9], ["지미추",95.67,1.51,6], ["큐라덴",95.71,1.60,7], ["메이크프렘",95.73,2.02,15], ["가그린",95.75,1.91,8], ["바이오코어유산균",95.75,1.28,8],
    ["위시어",95.75,0.46,8], ["닥터디퍼런트",95.80,1.79,5], ["유리아쥬",95.82,3.28,11], ["바이오힐보",95.82,1.94,22], ["랑방",95.83,1.11,12], ["마녀공장",95.83,1.19,12],
    ["라엘",95.85,1.95,13], ["제로이드",95.85,0.91,27], ["아크네스",95.86,0.69,7], ["멜라메이트",95.86,0.38,7], ["스튜디오17",95.92,1.85,13], ["리쥬란",95.94,1.48,16],
    ["메디힐",95.95,1.74,38], ["에스트라",95.98,1.35,41], ["아렌시아",96.00,2.12,5], ["키스미",96.00,2.28,6], ["헤라",96.00,1.56,10], ["나른",96.08,1.89,13],
    ["갸스비",96.09,1.92,11], ["스킨푸드",96.10,0.99,10], ["셀리맥스",96.11,1.83,9], ["라곰",96.12,1.13,8], ["레이비",96.17,0.75,6], ["센소다인",96.17,0.75,6],
    ["어노브",96.18,2.07,17], ["토리든",96.19,2.10,36], ["존바바토스",96.20,0.84,5], ["바이컬러",96.20,0.84,5], ["닥터지",96.21,1.69,28], ["쉬크",96.25,1.16,8],
    ["세타필",96.25,1.98,8], ["츠바키",96.25,0.71,8], ["라운드랩",96.25,1.65,51], ["쏘피",96.27,1.39,15], ["비욘드",96.29,1.38,7], ["네이처메이드",96.33,1.03,6],
    ["올리오",96.33,1.41,9], ["아리아나그란데",96.33,0.82,6], ["폴프랜즈",96.38,1.19,8], ["닥터텅스",96.40,2.41,5], ["힐링버드",96.40,0.89,5], ["비비랩",96.40,1.30,15],
    ["피지오겔",96.42,1.53,24], ["피노",96.43,0.79,7], ["포맨트",96.50,1.58,18], ["웰라쥬",96.56,0.88,9], ["디어스킨",96.56,2.13,9], ["리스테린",96.56,1.36,16],
    ["일리윤",96.57,1.55,30], ["세노비스",96.59,1.77,17], ["프롬랩스",96.60,1.14,5], ["바세린",96.60,3.21,5], ["캘빈클라인",96.60,1.07,10], ["센텔리안24",96.60,1.34,5],
    ["아임비타",96.60,1.14,5], ["아이디얼포맨",96.67,2.02,15], ["파티온",96.69,1.18,13], ["오브제",96.70,2.80,23], ["AHC",96.73,1.10,11], ["비레디",96.75,1.91,8],
    ["휴족시간",96.83,0.75,6], ["비페스타",96.86,1.35,7], ["스킨1004",97.00,1.22,5], ["아이오페",97.00,1.00,17], ["스너글",97.08,0.90,12], ["우르오스",97.27,0.65,11],
    ["센카",97.29,1.38,7], ["유기농본",97.29,0.49,7], ["VT",97.33,1.54,15], ["순수한면",97.38,1.69,8], ["아토팜",97.38,0.74,8], ["템포",97.40,0.89,5],
    ["CJ이너비",97.50,1.64,6], ["크리넥스",97.50,1.27,10], ["동아제약",97.50,1.38,6], ["고려은단",97.60,0.83,15], ["좋은느낌",97.60,0.76,25], ["뉴오리진",97.60,0.89,5],
    ["멕리듬",97.60,0.89,5], ["베르사체",97.60,0.55,5], ["덴프스",97.70,1.16,10], ["시루콧토",97.71,0.95,7], ["오쏘몰",97.71,0.49,7], ["정관장",97.71,0.76,7],
    ["질레트",97.79,0.92,19], ["커리쉴",97.80,1.30,5], ["폰즈",98.00,0.00,5], ["더마비",98.00,0.71,5], ["리뉴",98.00,0.63,6], ["옵티프리",98.57,0.79,7],
    ["온그리디언츠",98.60,1.14,5],
  ];
  ...
  function drawChurn() {
    ...
    const peak = Math.max(...churn.map((c) => c[1]));
    const maxY = Math.ceil((peak * 1.12) / 10) * 10;
    const yS = (v) => y1 - (v / maxY) * (y1 - y0);

    const tick = maxY > 60 ? 20 : 10;
    Array.from({ length: maxY / tick + 1 }, (_, i) => i * tick).forEach((v) => {
      const y = yS(v);
    ...
    const iMax = churn.findIndex((c) => c[1] === peak);
    const xMax = x0 + iMax * ((x1 - x0) / n) + bw / 2;
    txt(svg, xMax, yS(peak) - 10, `최다 ${peak}개`, { "text-anchor": "middle", "font-size": 12, fill: col.s3, "font-weight": 800 });
  ...
  function drawLag() {
    ...
    /* 축 상한은 전 시차의 최댓값에서 잡는다 — 하드코딩하면 값이 커졌을 때 막대가 잘린다 */
    const lagMax = Math.max(...Object.values(lagData).flatMap((o) => [o.fwd, o.rev]));
    const x0 = L, x1 = W - R, maxX = Math.ceil((lagMax * 1.08) / 5) * 5;
    const xS = (v) => x0 + (v / maxX) * (x1 - x0);
  ...
  function drawDiscCurve() {
    ...
    const lo = qty ? -0.6 : -0.8, hi = qty ? 0.8 : 1.4;
  ...
  function drawCfPath() {
    ...
    [60, 80, 100, 120, 140, 160, 180, 200].forEach((v) => {
      const y = yS(v);
    ...
    const act = cfPost["실제(Actual)"], nn = cfPost[cfBase];
  ...
  function drawRevBrand() {
    ...
    txt(svg, x1 - 6, y0 + 14, "ρ = −0.68 (p < 0.001)", { "text-anchor": "end", "font-size": 12, fill: col.s3, "font-weight": 750 });
  ...
  const GLOSSARY = {
    ...
    "silhouette": ["실루엣 계수란?",
      "군집화가 잘 됐는지 채점하는 -1 ~ +1 사이의 점수다. 대상 하나마다 '같은 무리 안 동료들과의 거리'와 '가장 가까운 남의 무리와의 거리'를 견줘, 남보다 동료가 뚜렷이 가까우면 1 에 가깝고 둘이 비슷하면 0 에 가까워진다. 0.7 이상이면 무리가 확실히 떨어져 있다는 뜻이고, 0.3 안팎이면 경계가 흐릿하다 — 무리가 틀렸다기보다 대상이 연속적으로 퍼져 있어 자연적인 틈이 없다는 신호다."],
    "seed": ["난수 시드란?",
      "난수를 만들어 내는 계산의 출발값이다. k-means 는 처음에 중심점을 무작위로 찍고 그 자리에서 수렴하므로, 출발값이 달라지면 결과가 달라질 수 있다. 시드를 바꿔 여러 번 돌려 보는 것은 '이 분할이 운 좋게 한 번 나온 그림인가, 어디서 출발해도 같은 그림인가'를 확인하는 절차다."],
    "ari": ["조정 랜드지수(ARI)란?",
      "두 번의 군집 결과가 얼마나 같은 분할인지를 재는 지표다. 상품을 둘씩 짝지어 '두 결과 모두에서 같은 무리였는가 / 모두에서 다른 무리였는가'를 세고, 우연히 맞아떨어질 몫을 빼서 조정한다. 1 이면 이름표만 다를 뿐 완전히 같은 분할, 0 이면 무작위로 나눈 것과 다를 바 없다는 뜻이다."],
  };
  ...
  function drawSale() {
    ...
    const ei = saleSeries.findIndex((d) => d[0] === saleEnd);
    const xEnd = ei >= 0 ? xS(ei) : x1;
    el("rect", { x: xS(si) - 3, y: y0, width: xEnd - xS(si) + 3, height: y1 - y0, fill: col.s2, opacity: 0.07 }, svg);
    ...
    if (ei >= 0) {
      el("line", { x1: xEnd, y1: y0, x2: xEnd, y2: y1, stroke: col.s2, "stroke-width": 1.6, "stroke-dasharray": "5 4" }, svg);
      txt(svg, xEnd + 7, y0 + 13, "세일 종료", { "font-size": 11.5, fill: col.s2, "font-weight": 750 });
    }

    saleSeries.forEach((d, i) => {
      if (i % 2 === 0 || i === n - 1 || i === si || i === ei) {
        const on = i === si || i === ei;
    ...
    [[2, col.s1, "쿠폰 부착률"], [1, col.s3, "평균 할인율"]].forEach(([idx, color, label]) => {
      ...
      saleSeries.forEach((d, i) => {
        ...
        hover(c, `${d[0]} · ${label}`, [[label, `${d[idx].toFixed(1)}%`, color],
          ["구간", i < si ? "세일 전" : (ei >= 0 && i > ei ? "세일 후" : "세일")]]);
      });
      const last = saleSeries[n - 1][idx];
      txt(svg, x1 + 12, yS(last) + 4, label, { "font-size": 12, fill: color, "font-weight": 750 });
    });

    txt(svg, (x0 + x1) / 2, H - 8, "카테고리 랭킹 전 상품 평균 · 개시일과 종료일 하루 만에 할인율과 쿠폰 부착률이 함께 계단식으로 이동한다", {
      "text-anchor": "middle", "font-size": 11.5, fill: col.muted,
    });
  }
  ...
  function drawAll() {
    ...
    const col = C();
    legend("lgEvent", [["쿠폰 부착 (3,343건)", col.s1, "line"], ["세일 부착 (1,103건)", col.s3, "line"]]);
