...
(function () {
  ...
  const catCorr = [
    ["취미/팬시", -0.26, -0.43, -0.03],
    ["메이크업", -0.28, -0.51, -0.00],
    ["홈리빙/가전", -0.31, -0.61, 0.12],
    ["구강용품", -0.31, -0.49, -0.13],
    ["바디케어", -0.31, -0.48, -0.18],
    ["스킨케어", -0.32, -0.50, -0.14],
    ["헬스/건강용품", -0.32, -0.46, -0.12],
    ["향수/디퓨저", -0.33, -0.59, -0.09],
    ["푸드", -0.34, -0.44, -0.22],
    ["더모 코스메틱", -0.35, -0.53, -0.05],
    ["헤어케어", -0.35, -0.50, -0.21],
    ["패션", -0.37, -0.67, -0.16],
    ["마스크팩", -0.38, -0.52, -0.29],
    ["맨즈에딧", -0.40, -0.53, -0.22],
  ...
  const ratingBins = [
    [0.0, 642], [4.1, 8], [4.2, 19], [4.3, 34], [4.4, 55], [4.5, 155], [4.6, 335], [4.7, 987], [4.8, 2202], [4.9, 2492],
  ];

  /* 그림 6. 표준화 영향력 (탄력성 절댓값) */
  const elasticity = [["리뷰 증가 속도 (판매량 대리지표)", 0.41, "s1"], ["실제 판매가격", 0.37, "s2"]];

  /* 표 7. 동일 상품 내 프로모션별 순위 변화율 (+ = 개선) */
  const promo = [
    ["쿠폰 배지", 13.7, 7.3], ["증정 배지", 9.3, 5.7],
    ["할인율 (1%p당)", 2.2, 12.2], ["세일 배지", -8.3, 1.9],
  ];

  /* 그림 8. 이벤트 스터디 — 평소 순위 대비 ln(순위) 편차 (음수 = 상위) */
  const eventDays = [-3, -2, -1, 0, 1, 2, 3];
  const eventSeries = {
    쿠폰: [0.043, 0.060, 0.055, -0.144, -0.037, -0.063, -0.038],
    세일: [0.097, 0.103, 0.130, -0.061, -0.125, -0.165, -0.082],
  };
  ...
  const cloudWords = [
    ["피부", 13176], ["느낌", 12159], ["향", 7024], ["꾸준히", 5873],
    ["자극", 4902], ["여름", 4798], ["촉촉", 4732], ["효과", 4568],
    ["부담", 4202], ["얼굴", 3826], ["재구매", 3816], ["제형", 3684],
    ["데일리", 3606], ["가격", 3549], ["아침", 3228], ["부드럽게", 3058],
    ["메이크업", 3023], ["끈적임", 2858], ["매일", 2782], ["화장", 2674],
    ["지속력", 2534], ["만족", 2461], ["시간", 2423], ["선크림", 2415],
    ["거품", 2403], ["성분", 2402], ["크림", 2354], ["자연스럽게", 2315],
    ["트러블", 2255], ["가성비", 2236], ["깔끔하게", 2120], ["가볍게", 2108],
    ["발림성", 2041], ["타입", 2023], ["구성", 2015], ["도움", 1965],
    ["관리", 1882], ["흡수", 1818], ["세안", 1807], ["냄새", 1779],
    ["나면", 1741], ["진정", 1725], ["부드럽고", 1713], ["용량", 1707],
    ["기획", 1685], ["편하고", 1662], ["수분", 1661], ["컬러", 1652],
  ];

  /* 그림 12. 소구 속성별 언급률 (%) — 전체 및 카테고리별 */
  const attrNames = ["보습·수분", "발림·제형", "자극·순함", "가격·가성비", "향", "끈적임·마무리", "재구매 의사", "지속력", "트러블·진정", "커버·발색"];
  const attrOverall = [19.9, 18.4, 16.6, 15.0, 14.1, 12.8, 12.5, 9.3, 9.3, 9.0];
  const attrProfile = [
    ["메이크업", 6774, [19.0, 19.4, 3.2, 9.6, 2.9, 16.2, 9.8, 26.4, 6.2, 31.5]],
    ["스킨케어", 6541, [45.9, 35.9, 30.9, 14.6, 5.6, 27.3, 15.6, 6.6, 25.5, 5.5]],
    ["마스크팩", 4697, [42.1, 15.7, 26.4, 15.6, 3.8, 11.9, 17.7, 6.2, 24.2, 18.4]],
    ["클렌징", 4643, [25.8, 20.4, 47.3, 13.2, 7.9, 7.9, 14.3, 2.5, 12.6, 1.5]],
    ["선케어", 4552, [32.4, 35.4, 17.8, 12.8, 4.6, 36.1, 11.1, 6.5, 10.0, 25.8]],
    ["헤어케어", 3988, [9.5, 20.8, 10.2, 14.3, 34.8, 12.7, 13.3, 10.2, 2.9, 3.2]],
    ["푸드", 3431, [1.4, 3.5, 3.1, 16.9, 6.0, 0.6, 14.4, 2.0, 0.5, 0.2]],
    ["바디케어", 3366, [28.1, 19.3, 19.3, 12.0, 45.1, 18.2, 11.3, 8.8, 6.7, 3.6]],
    ["건강식품", 3163, [2.2, 3.8, 2.1, 19.9, 4.0, 0.4, 14.5, 3.1, 0.7, 0.1]],
    ["위생용품", 3095, [9.6, 21.9, 26.6, 21.6, 17.4, 7.5, 15.5, 5.8, 1.5, 4.6]],
    ["맨즈에딧", 2816, [20.6, 17.0, 12.6, 13.1, 19.8, 19.4, 10.4, 10.9, 7.7, 8.4]],
    ["뷰티소품", 2773, [4.8, 16.3, 13.5, 20.4, 1.2, 3.0, 12.6, 5.0, 2.3, 9.0]],
    ["네일", 2755, [14.1, 20.3, 4.4, 14.4, 16.3, 8.5, 8.3, 14.9, 0.5, 6.0]],
    ["헬스/건강용품", 2636, [5.1, 4.8, 10.1, 16.4, 7.5, 2.6, 11.0, 4.4, 19.8, 8.0]],
    ["구강용품", 2555, [1.9, 5.1, 13.4, 17.5, 17.0, 1.4, 13.9, 7.4, 0.1, 0.5]],
    ["향수/디퓨저", 2532, [2.2, 4.3, 3.6, 17.0, 80.2, 4.3, 9.2, 32.5, 0.5, 0.4]],
    ["더모 코스메틱", 2228, [48.7, 33.9, 34.2, 13.2, 9.8, 21.9, 12.2, 7.0, 27.6, 5.6]],
    ["패션", 1781, [1.0, 5.3, 5.6, 15.9, 0.4, 1.3, 8.1, 1.7, 0.4, 6.3]],
    ["홈리빙/가전", 1290, [4.4, 3.5, 6.1, 15.9, 29.1, 1.2, 10.0, 6.4, 0.5, 0.4]],
    ["취미/팬시", 941, [0.4, 4.0, 1.2, 18.1, 0.6, 0.6, 6.2, 2.4, 0.1, 0.3]],
  ];
  ...
  const discTypes = [
    ["가속형", "s1", [0, -0.03, 0.05, 0.17, 0.40, 0.67], [0, 0.22, 0.44, 0.79, 1.15, 1.62], 14, 1004],
    ["데스밸리형", "s3", [0, -0.44, -0.34, -0.15, -0.03, 0.07], [0, -0.64, -0.39, -0.25, -0.01, -0.18], 14, 1109],
    ["포화형", "s2", [0, 0.23, 0.20, 0.33, 0.42, 0.29], [0, 0.24, 0.33, 0.48, 0.69, 0.53], 17, 1211],
    ["무반응·역행형", "neutralInk", [0, -0.19, -0.40, -0.38, -0.41, -0.47], [0, -0.05, -0.11, -0.04, -0.21, -0.44], 12, 796],
  ];

  /* 그림 10. 프로모션 온셋 전후 판매지수 — 실제와 반사실 (사전 평균 = 100) */
  const cfDays = ["D-7", "D-6", "D-5", "D-4", "D-3", "D-2", "D-1", "D+0", "D+1", "D+2", "D+3", "D+4"];
  const cfPre = [103.4, 92.0, 121.0, 101.9, 102.8, 109.2, 75.5];
  const cfPost = {
    "실제(Actual)": [122.4, 119.9, 103.8, 112.2, 103.6],
    "EARTH(MARS)": [117.0, 113.4, 104.9, 106.8, 100.1],
    "신경망(MLP)": [114.1, 109.7, 100.2, 102.6, 97.6],
    "시장대조(DiD)": [114.6, 108.9, 98.1, 101.7, 93.4],
    "ARIMA(1,0,0)": [112.2, 94.9, 105.9, 97.7, 103.3],
    "사전평균(무보정)": [100.0, 101.0, 101.0, 100.0, 100.7],
  };
  ...
  const revCat = [
    ["취미/팬시", 89.3, 6.1, 4.52, 25],
    ["네일", 91.8, 5.6, 2.69, 85],
    ["패션", 92.4, 4.8, 2.94, 105],
    ["헬스/건강용품", 93.5, 4.6, 1.80, 186],
    ["메이크업", 93.9, 4.2, 1.91, 198],
    ["선케어", 94.3, 3.7, 2.09, 196],
    ["구강용품", 94.5, 4.0, 1.40, 235],
    ["뷰티소품", 94.6, 3.8, 1.47, 220],
    ["푸드", 94.7, 3.6, 1.62, 215],
    ["향수/디퓨저", 95.1, 3.2, 1.59, 220],
    ["헤어케어", 95.1, 3.5, 1.28, 213],
    ["바디케어", 95.3, 3.4, 1.20, 197],
    ["홈리빙/가전", 95.3, 3.1, 1.56, 50],
    ["마스크팩", 95.4, 3.3, 1.33, 218],
    ["더모 코스메틱", 95.7, 3.0, 1.09, 203],
    ["클렌징", 95.8, 3.1, 1.05, 193],
    ["스킨케어", 95.8, 3.0, 1.13, 146],
    ["맨즈에딧", 96.0, 2.8, 0.93, 211],
    ["건강식품", 96.0, 3.0, 0.80, 274],
    ["위생용품", 96.2, 2.7, 0.89, 193],
  ];

  /* 그림 14. 브랜드별 긍정 비중 산포 — [브랜드, 긍정 비중%, 브랜드 내 상품 간 표준편차%p, 상품수] */
  const revBrand = [
    ["웰릭스",86.00,6.12,5], ["라이브오랄스",88.60,3.05,5], ["베리시",89.10,7.72,10], ["바나실",89.60,5.03,5], ["마르시끄",89.60,2.59,10], ["알로",90.67,2.34,6],
    ["위드샨",90.69,3.36,16], ["체이싱래빗",90.86,2.34,7], ["마른파이브",91.15,2.97,13], ["민티드",91.20,3.03,5], ["이지덤뷰티",91.33,4.08,6], ["센시안",91.45,5.30,11],
    ["해서린",92.00,4.00,5], ["리무브",92.12,2.53,8], ["에이딕트",92.12,2.95,8], ["디오디너리",92.40,1.67,5], ["셀라딕스",92.40,1.67,5], ["웨이크메이크",92.48,2.24,27],
    ["유시몰",92.56,3.75,27], ["비플레인",92.65,8.08,17], ["센녹",92.80,2.39,5], ["투크",92.80,3.90,5], ["파넬",92.83,3.25,6], ["크런틴",92.83,3.82,6],
    ["리브러쉬",92.88,2.30,8], ["케어플러스",92.90,5.16,20], ["반디",93.00,3.40,10], ["청미정",93.00,1.53,7], ["발란스핏",93.00,2.55,5], ["필리밀리",93.06,4.24,72],
    ["메디큐브",93.11,2.14,18], ["다슈",93.14,4.40,29], ["데싱디바",93.14,2.79,7], ["쏘내추럴",93.17,3.43,6], ["비너스",93.17,2.32,6], ["그린몬스터",93.17,2.71,6],
    ["힌스",93.29,1.98,7], ["나르카",93.33,1.21,6], ["푸드올로지",93.38,2.79,24], ["컬러그램",93.50,1.38,6], ["네이밍",93.57,2.07,7], ["식물나라",93.58,2.87,31],
    ["grn+",93.60,1.67,5], ["케라스타즈",93.62,1.85,8], ["나르시소 로드리게즈",93.67,2.66,6], ["뷰센",93.75,3.31,12], ["아뜰리에페이",93.75,2.71,8], ["테일러",93.80,2.68,5],
    ["프롬리에",93.80,2.59,5], ["낫포유",93.80,1.64,5], ["나트라케어",93.80,2.49,5], ["프로티원",93.83,3.82,6], ["코스노리",93.86,3.24,7], ["페리오",93.91,2.91,11],
    ["플르부아",94.00,1.67,6], ["칼로(Kalo)",94.00,1.22,5], ["퓌",94.00,1.69,8], ["코자아",94.00,2.45,6], ["박준뷰티랩",94.00,2.55,9], ["일소",94.08,2.02,12],
    ["플라이밀",94.14,2.97,7], ["낫띵베럴",94.20,1.30,5], ["홀리카홀리카",94.20,3.03,5], ["아리얼",94.22,4.49,9], ["라운드어라운드",94.25,2.05,12], ["로레알",94.33,2.35,9],
    ["바이오던스",94.33,2.15,12], ["라로슈포제",94.38,2.17,32], ["테라브레스",94.42,5.14,12], ["아누아",94.42,2.22,33], ["정샘물",94.43,1.62,7], ["더툴랩",94.43,1.27,7],
    ["달바",94.47,3.00,15], ["유세린",94.47,1.85,15], ["에스쁘아",94.50,2.33,8], ["메디필",94.50,2.14,8], ["에뛰드",94.54,2.37,13], ["피카소",94.57,2.77,14],
    ["아떼",94.57,1.72,7], ["넥스케어",94.57,3.99,7], ["리터니티",94.60,2.61,5], ["스트라이덱스",94.60,1.95,5], ["팁토우",94.60,1.95,5], ["바른생각",94.64,2.06,14],
    ["탄탄",94.67,1.51,6], ["조선미녀",94.67,1.87,9], ["투쿨포스쿨",94.67,1.75,6], ["버버리",94.67,2.34,6], ["올리브영",94.70,3.36,37], ["브링그린",94.70,1.98,27],
    ["보다나",94.71,3.73,7], ["롬앤",94.80,2.10,10], ["크리스탈",94.80,0.45,5], ["이즈앤트리",94.80,1.30,5], ["동국제약",94.83,3.43,6], ["티젠",94.89,2.26,9],
    ["존슨즈",95.00,2.68,6], ["한율",95.00,0.71,9], ["어뮤즈",95.00,2.35,5], ["이지듀",95.00,1.85,8], ["니베아",95.00,1.67,6], ["라보에이치",95.00,1.00,11],
    ["미쟝센",95.00,2.07,15], ["클린",95.00,1.69,8], ["구달",95.06,2.04,18], ["딜라이트 프로젝트",95.10,2.33,31], ["토르홉",95.12,1.64,8], ["바이탈뷰티",95.12,1.64,8],
    ["루치펠로",95.19,2.26,16], ["올더베러",95.20,1.47,20], ["그라펜",95.20,3.68,10], ["듀이트리",95.23,2.42,13], ["헤트라스",95.25,1.58,8], ["화이트",95.27,3.10,11],
    ["해피바스",95.29,2.29,7], ["프리메라",95.29,0.76,7], ["닥터포헤어",95.30,1.25,10], ["라네즈",95.33,3.04,9], ["아로마티카",95.38,1.41,8], ["아비브",95.38,2.33,21],
    ["넘버즈인",95.39,1.27,23], ["오니스트",95.40,1.52,5], ["코링코",95.43,0.53,7], ["아벤느",95.50,1.43,10], ["코스알엑스",95.50,3.02,8], ["바이오가",95.50,1.93,8],
    ["바닐라코",95.53,2.82,19], ["클리오",95.55,1.04,11], ["이니스프리",95.55,3.64,11], ["이너시아",95.56,1.94,9], ["아비노",95.60,1.67,5], ["멈칫",95.60,1.34,5],
    ["락토핏",95.60,3.50,10], ["바이오힐보",95.62,2.13,21], ["가그린",95.67,2.16,6], ["아크네스",95.67,0.52,6], ["셀퓨전씨",95.67,1.41,9], ["에스트라",95.69,1.55,36],
    ["페리페라",95.71,1.11,7], ["바이오코어유산균",95.75,1.28,8], ["라엘",95.77,2.05,13], ["제로이드",95.80,0.91,25], ["오랄비",95.80,1.48,5], ["위시어",95.80,0.45,5],
    ["아렌시아",95.80,2.05,5], ["닥터디퍼런트",95.80,1.79,5], ["큐라덴",95.80,0.84,5], ["에스네이처",95.81,1.12,21], ["바이오더마",95.85,1.92,33], ["멜라메이트",95.86,0.38,7],
    ["마녀공장",95.90,1.29,10], ["랑방",95.91,1.22,11], ["스튜디오17",95.92,1.83,12], ["지미추",96.00,1.41,6], ["메이크프렘",96.00,1.18,11], ["존바바토스",96.00,0.71,5],
    ["올리오",96.00,1.41,7], ["키스미",96.00,2.28,6], ["나른",96.08,1.88,12], ["메디힐",96.12,1.75,33], ["폴프랜즈",96.12,0.99,8], ["라곰",96.12,1.46,8],
    ["리쥬란",96.13,1.60,15], ["세타필",96.14,2.12,7], ["덴티스테",96.17,1.38,18], ["쏘피",96.17,1.40,12], ["레이비",96.17,0.75,6], ["닥터자르트",96.20,0.84,5],
    ["바이컬러",96.20,0.84,5], ["피노",96.20,0.84,5], ["아리아나그란데",96.20,0.84,5], ["어노브",96.21,2.12,14], ["갸스비",96.22,1.79,9], ["쉬크",96.25,1.16,8],
    ["토리든",96.28,2.08,32], ["오브제",96.33,3.05,18], ["유리아쥬",96.33,3.24,9], ["스킨푸드",96.33,0.52,6], ["헤라",96.38,1.30,8], ["비욘드",96.40,1.82,5],
    ["아이디얼포맨",96.43,2.31,14], ["라운드랩",96.44,1.52,45], ["캘빈클라인",96.44,1.01,9], ["닥터지",96.46,1.44,24], ["비비랩",96.46,1.56,13], ["리스테린",96.46,1.39,13],
    ["네이처메이드",96.50,0.84,6], ["디어스킨",96.50,2.27,8], ["셀리맥스",96.50,1.85,8], ["츠바키",96.50,1.22,6], ["피지오겔",96.55,1.28,20], ["일리윤",96.56,1.53,25],
    ["닥터텅스",96.60,2.30,5], ["포맨트",96.71,1.45,17], ["웰라쥬",96.71,0.95,7], ["파티온",96.73,1.19,11], ["비페스타",96.80,1.48,5], ["프롬랩스",96.80,1.30,5],
    ["휴족시간",96.83,0.75,6], ["AHC",96.89,1.05,9], ["세노비스",96.93,1.33,14], ["스킨1004",97.00,1.22,5], ["아이오페",97.06,1.24,16], ["스너글",97.08,0.90,12],
    ["비레디",97.14,1.35,7], ["아토팜",97.17,0.98,6], ["우르오스",97.27,0.79,11], ["유기농본",97.29,0.49,7], ["센카",97.33,1.51,6], ["뉴오리진",97.40,0.55,5],
    ["동아제약",97.50,1.05,6], ["고려은단",97.57,0.76,14], ["순수한면",97.60,1.82,5], ["크리넥스",97.60,1.52,5], ["좋은느낌",97.62,0.77,24], ["질레트",97.71,0.92,17],
    ["오쏘몰",97.71,0.49,7], ["정관장",97.71,0.76,7], ["시루콧토",97.71,0.95,7], ["덴프스",97.75,0.89,8], ["더마비",97.80,0.45,5], ["VT",98.00,1.41,10],
    ["옵티프리",98.50,0.55,6],
  ];
